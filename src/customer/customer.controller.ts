...
import { ApiDataResponse, ApiPaginatedResponse } from '../common/decorators/response.decorator.js';
...
export class CustomerController {
  ...
  async findAll(
    ...
    const mappedData = result.data.map((entity) => this.customerMapper.entityToResponseDto(entity));
  ...
  async findOne(@Param('id') id: string): Promise<DataResponseDto<CustomerResponseDto>> {
    const customer = await this.customerService.getById(id);
    return new DataResponseDto(this.customerMapper.entityToResponseDto(customer));
  }
}
