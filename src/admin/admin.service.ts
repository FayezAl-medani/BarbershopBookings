...
export class AdminService implements IAdminService {
  ...
  async registerAdminUser(payload: AdminRegisterDto): Promise<AdminEntity> {
    const existsByEmail = await this.adminRepository.existsByEmail(payload.email);
    if (existsByEmail) {
    ...
    const existsByPhone = await this.userRepository.existsByPhoneNumber(payload.phoneNumber);
    if (existsByPhone) {
      throw new ConflictException(
