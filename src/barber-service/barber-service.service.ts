...
export class BarberServiceService implements IBarberServiceService {
  ...
  async addServiceToBarber(
    ...
    const exists = await this.barberServiceRepository.exists(
      barberId,
      serviceId,
    );
    if (exists) {
    ...
    const result = await this.barberServiceRepository.create(
      barberId,
      serviceId,
    );
  ...
  async getBarberServices(barberId: string): Promise<BarberServiceEntity[]> {
    const results =
      await this.barberServiceRepository.findAllByBarberId(barberId);
    return results.map(
      (r) =>
