...
export class UserCredentialService implements IUserCredentialService {
  ...
  async create(payload: UserCredentialCreateDto): Promise<UserCredentialEntity> {
  ...
  async getByUserId(userId: string): Promise<UserCredentialEntity | null> {
    ...
    return credential ? this.userCredentialMapper.modelToEntity(credential) : null;
  }

  async getByUserIdAndMethod(userId: string, method: string): Promise<UserCredentialEntity | null> {
    const credential = await this.userCredentialRepository.findByUserIdAndMethod(userId, method);
    return credential ? this.userCredentialMapper.modelToEntity(credential) : null;
  }

  async updateById(id: string, data: Partial<UserCredentialCreateDto>): Promise<UserCredentialEntity> {
