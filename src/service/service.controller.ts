...
export class ServiceController {
  ...
  async findAll(
    ...
    @SortingParams(['name', 'price', 'duration', 'createdAt']) sort: SortingParam | null,
  ): Promise<PagingDataResponseDto<ServiceResponseDto>> {
    const result = await this.serviceService.findAllPaging(filter, pagingArgs, sort);
    const mappedData = result.data.map((entity) => this.serviceMapper.entityToResponseDto(entity));
  ...
  async findOne(@Param('id') id: string): Promise<DataResponseDto<ServiceResponseDto>> {
  ...
  async create(@Body() payload: ServiceCreateDto): Promise<DataResponseDto<ServiceResponseDto>> {
